...
- [ ] If test files were added, update the README Status test-file count
...
- **Stats are concrete.** "1915 tests" not "comprehensive test suite". "31K lines" not "large codebase".
...
- Prefer small, focused modules over monoliths. `server/index.js` is 148 lines of wiring — keep it that way.
...
| What | Where |
|------|-------|
| Server entry | `server/index.js` (148 lines, thin wiring) |
| Routes | `server/routes/` (23 modules — 8 top-level + 15 project domain) |
| Services | `server/services/` (52 modules) |
| WebSocket | `server/ws/` (3 modules) |
| Backends | `server/backends/` (6 runners) |
| Swarm | `server/swarm/` (6 runners) |
